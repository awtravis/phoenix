...
InputParameters validParams<WidmanstattenMaterial>()
{
  ...
  params.addParam<Real>("anisotropy_strength", 0.05, "Strength of the anisotropy (typically < 0.05)");
  params.addParam<unsigned int>("mode_number", 2, "Mode number for anisotropy");
...
WidmanstattenMaterial::WidmanstattenMaterial(const InputParameters & parameters) :
    Material(parameters),
    ...
    _grad_op(coupledGradient("op"))
{
  ...
  if (_mesh.dimension() != 2)
    mooseError("WidmanstattenMaterial requires a two-dimensional mesh.");
}
